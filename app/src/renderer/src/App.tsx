...
type EditorWithFit = Editor & {
  ...
  zoomToBounds?: (b: Bounds, opts?: { inset?: number; targetZoom?: number } & TLCameraMoveOptions) => Editor
}
...
function backToContentSafe(editor: Editor): void {
  ...
  } else if (typeof ed.getContentBounds === 'function' && typeof ed.zoomToBounds === 'function') {
    const b = ed.getContentBounds()
    if (isUsableBounds(b)) ed.zoomToBounds(b, { inset: 64, animation: { duration: 200 } })
  } else {
    const cam = editor.getCamera()
    editor.setCamera({ x: cam.x, y: cam.y, z: 0.5 }, { immediate: true })
  }
  const cam = editor.getCamera()
  if (cam.z < 0.4) editor.setCamera({ x: cam.x, y: cam.y, z: 0.4 }, { immediate: true })
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const off = api.onNotice((n: OverlayNotice) => {
      switch (n.kind) {
        case 'tab-limit': toast.error(`Tab limit reached (${n.max}).`); break
        case 'popup-suppressed': toast('Max browser windows from links reached.'); break
        case 'external-auth': toast('Google sign-in was opened in your default browser.'); break
        case 'tab-crashed': toast.error('This tab crashed and was closed.'); break
        case 'nav-error': toast.error(`Navigation failed (${n.code}): ${n.description}`); break
        case 'screen-share-error': toast.error(`Screen share error: ${n.message}`); break
        case 'media-denied': toast('Permission denied.'); break
      }
    })
    return off
  }, [])
